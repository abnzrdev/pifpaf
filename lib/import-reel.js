...
export async function refreshStaleReelsForUser({
  userId,
  cutoff = new Date(Date.now() - 6 * 60 * 60 * 1000),
  fetcher = fetchReel,
  repository = getRepository(),
}) {
  const [stale] = await repository.getStaleReels(userId, cutoff)
  if (!stale) return { refreshed: 0 }

  try {
    await repository.upsertReel(userId, await fetcher(stale.url))
    return { refreshed: 1 }
  } catch (error) {
    return { refreshed: 0, error: publicMessage(error) }
  }
}
